...
export default function Dashboard() {
  ...
  const formatAuditError = (
    payload: (AuditError & { retryAfterSeconds?: number }) | null,
    status: number
  ) => {
    ...
    switch (errorCode) {
      ...
      default:
        return resolveApiError(
          payload,
          status >= 500
            ? "The audit backend returned an error."
            : "Unable to contact the audit backend."
        );
    }
  };
  ...
  const loadRecentScans = async () => {
    try {
      ...
      if (!resp.ok) {
        setHistoryError(
          await readJson<{ error?: string; details?: string }>(resp).then(
            (payload) => resolveApiError(payload, "Unable to load recent scans.")
          )
        );
        return;
      }
      ...
      setRecentScans([]);
      setHistoryError(
        getFetchErrorMessage(error, "Unable to load recent scans.")
      );
    }
  };
  ...
  const handleStartAudit = async () => {
    if (!config.apiKey) {
      setErrorMessage(
        "No API key is loaded yet. Generate one in API Settings first."
      );
      setState("error");
      return;
    }
    ...
    try {
      ...
      if (!resp.ok) {
        const errorData =
          (await readJson<AuditError & { retryAfterSeconds?: number }>(resp)) ||
          null;
      ...
      setErrorMessage(
        getFetchErrorMessage(
          error,
          "Unable to contact the audit backend. Try again later."
        )
      );
      setState("error");
    }
  ...
  const resultSummary = useMemo(() => {
    const risk = (result?.analysis?.riskLevel || "UNKNOWN")
      .toString()
      .toUpperCase();
    const riskLabel =
      risk === "CRITICAL"
        ...
        ? "moderate"
        : "low";
    const summary = `This host has ${riskLabel} quantum vulnerability. ${
      result?.analysis?.quantumVulnerable
        ? "Not quantum-safe."
        : "Quantum-safe algorithms may be present."
    }`;
    return { risk, summary };
  }, [result]);

  const scores = useMemo(() => {
    const backendVuln = Number.isFinite(
      Number(result?.analysis?.vulnerabilityScore)
    )
      ? Number(result?.analysis?.vulnerabilityScore)
      : null;
    const backendQuantum = Number.isFinite(
      Number(result?.analysis?.breakdown?.quantumScore)
    )
      ? Number(result?.analysis?.breakdown?.quantumScore)
      : null;
    const backendClassical = Number.isFinite(
      Number(result?.analysis?.breakdown?.classicalScore)
    )
      ? Number(result?.analysis?.breakdown?.classicalScore)
      : null;

    const vulnerability =
      backendVuln !== null ? Math.max(0, Math.min(100, backendVuln)) : 95;

    const fallbackQuantum = Math.round(
      Math.max(0, Math.min(100, 100 - vulnerability))
    );

    const ks = Number(result?.scan?.certInfo?.publicKeySize || 0);
    const fallbackClassical =
      ks > 0
        ? Math.round(Math.max(0, Math.min(100, (ks / 4096) * 100)))
        : (result?.analysis?.riskLevel || "").toUpperCase() === "CRITICAL"
        ? 60
        : 85;

    const quantum = backendQuantum !== null ? backendQuantum : fallbackQuantum;
    const classical =
      backendClassical !== null ? backendClassical : fallbackClassical;
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
          ...
          <div className="space-y-5">
            ...
            <div>
              <h1 className="text-2xl md:text-4xl font-bold text-white leading-tight">
                Scan a site, understand the risk, and keep every result in one
                place.
              </h1>
              <p className="mt-3 text-sm md:text-base text-gray-400 max-w-2xl">
                Enter a website URL and the dashboard will inspect TLS,
                summarize the exposure, and store the audit so the history stays
                visible even when keys change.
              </p>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {QUICK_POINTS.map((point) => (
                <div
                  key={point}
                  className="rounded-xl border border-[#1e2532] bg-[#0B0E14] px-4 py-3 text-sm text-gray-300"
                >
                  {point}
                </div>
              ))}
          ...
          <div className="rounded-2xl border border-[#1e2532] bg-[#0B0E14] p-4 md:p-5 space-y-4">
            ...
            <div className="space-y-2 text-sm text-gray-300">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#00FF94]" /> Audit
                stored in MongoDB
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#00FF94]" /> Scan
                history survives key rotation
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#00FF94]" /> Results
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-400">
                Use a valid public URL. The audit will pull TLS certificate data
                and score the exposure.
              </p>
            </div>
            ...
            <div className="flex-1">
              ...
              {errorType === "invalid_url" && (
                <motion.p
                  ...
                  className="text-xs text-[#FF4D4D] mt-2 flex items-center gap-1"
                >
      ...
      {state === "empty" && (
        <div className="grid grid-cols-1 xl:grid-cols-[0.95fr_1.05fr] gap-4 md:gap-6">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-[#151921] border border-[#1e2532] rounded-xl p-8 md:p-10 text-left space-y-4"
          >
            <Shield className="w-12 h-12 md:w-14 md:h-14 text-[#00A3FF]" />
            <div>
              <h3 className="text-lg md:text-xl font-bold text-white mb-2">
                What this scan page does
              </h3>
              <p className="text-sm md:text-base text-gray-400">
                It keeps the first screen focused: enter a URL, run the audit,
                then review the result cards and stored history without hunting
                through hidden controls.
              </p>
            </div>
            <div className="space-y-2 text-sm text-gray-300">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#00FF94]" /> Clear input
                and one action button
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#00FF94]" /> Error
                messages show the backend reason
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#00FF94]" /> Recent
                scans stay visible below
              </div>
            </div>
          </motion.div>
      ...
      {state === "result" && result && (() => {
        ...
        const riskBg =
          riskLevel === "CRITICAL"
            ? "bg-[#FF4D4D]/20 border-[#FF4D4D]/30"
            : riskLevel === "HIGH"
            ? "bg-[#FF8C00]/20 border-[#FF8C00]/30"
            : riskLevel === "MEDIUM"
            ? "bg-[#FFB84D]/20 border-[#FFB84D]/30"
            : "bg-[#00FF94]/20 border-[#00FF94]/30";
        ...
        const issuer =
          certInfo.issuer?.organizationName ||
          certInfo.issuer?.commonName ||
          "Unknown";
        const subject =
          certInfo.subject?.commonName || result?.scan?.host || "Unknown";
        const notAfter = certInfo.notAfter ? new Date(certInfo.notAfter) : null;
        const daysLeft = notAfter
          ? Math.max(
              0,
              Math.ceil((notAfter.getTime() - Date.now()) / 86400000)
            )
          : null;
        const expiryColor =
          daysLeft !== null
        ...
        const classicalScore =
          analysis.breakdown?.classicalScore ?? scores.classical;
        const quantumScore =
          analysis.breakdown?.quantumScore ?? scores.quantum;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <BentoCard delay={0} className="col-span-1 md:col-span-2">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <h3 className="text-xl md:text-2xl font-bold text-white break-all">
                      {subject}
                    </h3>
                  </div>
                ...
                {notAfter && (
                  <div className="mt-3 text-xs text-gray-500">
                    Valid until:{" "}
                    {notAfter.toLocaleDateString(undefined, {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
              ...
              <BentoCard delay={0.2} className="col-span-1 md:col-span-2">
                <div className="flex flex-col lg:flex-row items-start justify-between mb-6 gap-4">
                  <div className="flex-1">
                    <p className="text-gray-400 text-xs md:text-sm mb-1">
                      Quantum vulnerability assessment — {subject}
                    </p>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                      ...
                      <div
                        className={`px-3 py-1 border rounded-full ${riskBg}`}
                      >
                        <span
                  ...
                  <div className="flex-1 max-w-full lg:max-w-md space-y-3">
                    <p className="text-sm md:text-base text-gray-300">
                      {analysis.quantumVulnerable
                        ? `${subject} uses ${algo}${
                            keySize ? `-${keySize}` : ""
                          } which is vulnerable to quantum computing attacks via Shor's algorithm. Immediate PQC migration planning is recommended.`
              ...
              <BentoCard delay={0.3} className="col-span-1 md:col-span-2">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-gray-400 text-xs md:text-sm mb-1">
                      NIST PQC migration recommendation
                    </p>
                    <h3 className="text-xl md:text-2xl font-bold text-white">
                      ML-KEM-768
                    </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="bg-[#0B0E14] rounded-lg p-4 border border-[#1e2532]">
                    ...
                    <div className="text-sm font-bold text-[#00A3FF]">
                      Level 3 (AES-192)
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      768-bit parameter
                ...
                {recommendations.length > 0 ? (
                  <div className="mt-4 space-y-2">
                    {recommendations.map((rec: string, i: number) => (
                      <div
                        key={i}
                        className="p-3 bg-[#00FF94]/10 border border-[#00FF94]/20 rounded-lg"
                      >
                        <p className="text-xs md:text-sm text-gray-300">
                          <span className="font-bold text-[#00FF94]">→ </span>
                          {rec}
                        </p>
                      </div>
                    ))}
                  ...
                  <div className="mt-4 p-4 bg-[#00FF94]/10 border border-[#00FF94]/20 rounded-lg">
                    <p className="text-xs md:text-sm text-gray-300">
                      ...
                      Implement hybrid TLS with ML-KEM-768 + X25519 to keep
                      both quantum-safe and classical security during transition.
                    </p>
                  </div>
                )}
              </BentoCard>
        ...
        <BentoCard>
          <div className="text-center py-4 md:py-6">
            ...
            <div className="mt-4">
              <button
                onClick={handleReset}
                className="px-4 py-2 bg-[#00A3FF] text-white rounded-lg"
              >
                Back
              </button>
            </div>
